...
# Load delays, generated by OLS search
delays = [list(delay_set) for delay_set in np.load(os.path.join(load_path, "best_delays.npy"))]
...
# Calculate performance curves for different number of branches
delays_whole = deepcopy(delays)
pereto_curve_train, pereto_curve_test = [], []
for i_model in range(len(delays)):

    delays = delays_whole[:i_model + 1]
    ...
    _, best_criterion = train_ls(model, train_dataset, validate_dataset, test_dataset, loss, 
                                        quality_criterion, config, batch_to_tensors, chunk_num, 
                                        load_path, exp_name, weight_names)
    ...
    with torch.no_grad():
        ...
        for j_dataset, dataset in enumerate(dataset_all):
            ...
            if j_dataset == 0:
                pereto_curve_train.append(NMSE)
            else:
                pereto_curve_test.append(NMSE)
            ...
            if j_dataset == 0:
                np.save(os.path.join(load_path, f'y_{i_model + 1}_branch.npy'), y_full_numpy)
                np.save(os.path.join(load_path, f'd_{i_model + 1}_branch.npy'), d_full_numpy)
                np.save(os.path.join(load_path, f'x_{i_model + 1}_branch.npy'), x_full_numpy)
            elif j_dataset == 1:
                np.save(os.path.join(load_path, f'y_test_{i_model + 1}_branch.npy'), y_full_numpy)
                np.save(os.path.join(load_path, f'd_test_{i_model + 1}_branch.npy'), d_full_numpy)
                np.save(os.path.join(load_path, f'x_test_{i_model + 1}_branch.npy'), x_full_numpy)
        np.save(os.path.join(load_path, f'pareto_curve_train.npy'), np.array(pereto_curve_train))
        np.save(os.path.join(load_path, f'pareto_curve_test.npy'), np.array(pereto_curve_test))